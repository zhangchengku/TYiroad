...
public class DbQualityActivity extends MVPBaseActivity<DbQualityContract.View, DbQualityPresenter> implements DbQualityContract.View {
    ...
    private ArrayList<String> listHGDS = new ArrayList<>();
    ...
    private void initdata() {
        ...
        edJcfw.setText(DBQualityData.getJCFW());
        ...
        listHGDS.addAll(Arrays.asList(DBQualityData.getHGDS().split(",")));
        for (int i = 0; i < listCSXMID.size(); i++) {
            ...
            jcxmBean.setHGDS(listHGDS.get(i));
    ...
    private void initpop() {
        XmmcPop = new DbQualityPopupWindow(this, "请选择项目名称", XmmcResult, new DiseaseNewSelectObjectListener() {
            ...
            public void selectPosition(int position) {
                ...
                mPresenter.getFbgcData2(XmmcInfo.getGUID_OBJ());
            }
        });
        FbgcPop = new DbQualityPopupWindow(this, "请选择分部工程", FbgcResult, new DiseaseNewSelectObjectListener() {
            ...
            public void selectPosition(int position) {
                ...
                mPresenter.getFxgcData2(XmmcInfo.getGUID_OBJ(), FbgcInfo.getGUID_OBJ());
            }
        });
    ...
    public void getXmmcDatas(List<XmmcBean.DATABean> videoVos2) {
        if (videoVos2.size() > 0) {
            ...
            for (int i = 0; i < videoVos2.size(); i++) {
                if (videoVos2.get(i).getGUID_OBJ().equals(DBQualityData.getXmmcID())) {
                    XmmcInfo = videoVos2.get(i);
                }
            }
            for (int i = 0; i < videoVos2.size(); i++) {
    ...
    public void getFbgcDatas(List<FbgcBean.DATABean> videoVos2) {
        if (videoVos2.size() > 0) {
            ...
            for (int i = 0; i < videoVos2.size(); i++) {
                if (videoVos2.get(i).getGUID_OBJ().equals(DBQualityData.getFbgcID())) {
                    FbgcInfo = videoVos2.get(i);
                }
            }
        }
    ...
    @Override
    public void getFbgcDatas2(List<FbgcBean.DATABean> videoVos2) {
        FbgcResult.clear();
        FbgcInfos.clear();
    ...
    public void getFxgcDatas(List<FxgcBean.DATABean> videoVos2) {
        if (videoVos2.size() > 0) {
            ...
            for (int i = 0; i < videoVos2.size(); i++) {
                if (videoVos2.get(i).getGUID_OBJ().equals(DBQualityData.getFxgcID())) {
                    FxgcInfo = videoVos2.get(i);
                }
            }
        }
        if (loadDataDialog != null && loadDataDialog.isShowing()) {
    ...
    @Override
    public void getFxgcDatas2(List<FxgcBean.DATABean> videoVos2) {
        FxgcResult.clear();
        FxgcInfos.clear();
        layXm.setVisibility(View.GONE);
        listview.setVisibility(View.GONE);
