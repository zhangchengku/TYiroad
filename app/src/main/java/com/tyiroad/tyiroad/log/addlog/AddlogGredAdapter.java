    ...
    public View getView(int position, View convertView, final ViewGroup parent) {
        ...
        if (showType == 1) {
            ...
            if (position == listPicture.size() - 1) {
                vh.pictureCloseImg.setVisibility(View.GONE);
                if (context instanceof AddLogActivity) {
                    vh.pictureImg.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            if (minePopupWindow == null) {
                                minePopupWindow = new MinePopupWindow((AddLogActivity) context, itemOnClick);
                            }
                            minePopupWindow.showAtLocation(parent, Gravity.BOTTOM, 0, 0);
                            Utils.hideInputWindow((AddLogActivity)context);
                        }
                    });
                }
            } else {
...
//                    context.startActivity(intent);
    ...
    public void getPicFromPhoto() {
        ...
        ((AddLogActivity) context).startActivityForResult(intent, CHOOSE_PICTURE_CODE);
    }
    ...
    public void getPicFromCamera() {
        ...
        ((AddLogActivity) context).setCameraPath(path);
        ((AddLogActivity) context).setChildViewPosition(childViewPosition);
        ((AddLogActivity) context).startActivityForResult(intent, CAMERA_CODE);
    }
