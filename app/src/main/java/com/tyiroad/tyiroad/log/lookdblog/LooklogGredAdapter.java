...
public class LooklogGredAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, final ViewGroup parent) {
        ...
        if (showType == 1) {
            ...
            if (position == listPicture.size() - 1) {
                vh.pictureCloseImg.setVisibility(View.GONE);
                if (context instanceof LookDBLogActivity) {
                    vh.pictureImg.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            if (minePopupWindow == null) {
                                minePopupWindow = new MinePopupWindow((LookDBLogActivity) context, itemOnClick);
                            }
                            minePopupWindow.showAtLocation(parent, Gravity.BOTTOM, 0, 0);
                            Utils.hideInputWindow((LookDBLogActivity)context);
                        }
                    });
                }
            } else {
    ...
    public void getPicFromPhoto() {
        ...
        ((LookDBLogActivity) context).startActivityForResult(intent, CHOOSE_PICTURE_CODE);
    }
    ...
    public void getPicFromCamera() {
        ...
        ((LookDBLogActivity) context).setCameraPath(path);
        ((LookDBLogActivity) context).setChildViewPosition(childViewPosition);
        ((LookDBLogActivity) context).startActivityForResult(intent, CAMERA_CODE);
    }
